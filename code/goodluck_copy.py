...
def train_and_predict(line=None, model=None,which=None):
    ...
    #selected_items_number_list=range(100,2000,200)
    selected_items_number_list=1100
    ...
    if line=='offline':
        # 挑选合适的提交个数
        if isinstance(selected_items_number_list,list):
            for selected_items_number in selected_items_number_list:
                ...
                print 'Score:', Score
        # 生成线下文件
        elif isinstance(selected_items_number_list,int):
            selected_items_number=selected_items_number_list
            print 'selected_items_number:', selected_items_number
            tmp = [tuple(i) for i in tmp1.iloc[:selected_items_number][['user_id', 'sku_id']].values]
            user_existed = []
            res = []
        ...
        for i in tmp:
            if i[0] not in user_existed:
                ...
                user_existed.append(i[0])
        pd.DataFrame(res).to_csv(line + '.csv', index=False, header=['user_id', 'sku_id'])
        print line + '_{}.csv' + ' is saved'

def merge_and_judge(line=None):
    os.chdir('../outcome/'+line)
    res0,res1,res2=set([tuple(i) for i in pd.read_csv(line+'_0.csv').values]), \
                   set([tuple(i) for i in pd.read_csv(line + '_1.csv').values]), \
                   set([tuple(i) for i in pd.read_csv(line + '_2.csv').values])
    res=pd.DataFrame(list(res0+res1+res2),columns=['user_id', 'sku_id'])
    res.to_csv(line+'.csv')
    if line=='offline':
        res=res.values
        '''
...
if __name__ == '__main__':
    ...
    #train_and_predict(line='offline', model='xgb',which=2)
    #os.chdir('/home/fengyufei/PycharmProjects/jd_competition/code')
